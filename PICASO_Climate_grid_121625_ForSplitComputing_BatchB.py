...
def _resolve_case_file(case_ref: str) -> str:
    """
    Resolve case-list file from a basename or full path.
    Search order:
      1) as provided
      2) as provided + '.npy'
      3) results/<name>
    ...
    raise FileNotFoundError(
        f"Could not find case-list file from reference: '{case_ref}'\n"
        f"Searched:\n" + "\n".join(f"  {c}" for c in candidates)
    )

def _load_cases_from_file(case_ref: str) -> np.ndarray:
    """
    Load explicit cases from .npy, .json, or .csv.
    Expected row format: [rad, metallicity, tint, semi_major_AU, ctoO]
    Returns float array of shape (N, 5).
    """
    ...
    if ext == ".npy":
        ...
        arr = np.loadtxt(case_file, delimiter=",", dtype=float)
    else:
        raise ValueError(f"Unsupported case-list extension: '{ext}'. Use .npy, .json, or .csv.")
    ...
    if arr.ndim != 2 or arr.shape[1] != 5:
        raise ValueError(
            f"Expected case-list shape (N, 5), got {arr.shape}.\n"
            f"Row format must be: [rad, metallicity, tint, semi_major_AU, ctoO]"
        )

    logging.info("Loaded explicit case list: %s  shape=%s", case_file, arr.shape)
...
def PICASO_climate_model(x):
    ...
    # Supports both:
    #   x = [rad, metal, tint, semi, ctoO]         <- normal 5-axis gridvals
    #   x = ([rad, metal, tint, semi, ctoO],)       <- one-element wrapper from make_grid when gridvals=(cases,)
    if len(x) == 1 and hasattr(x[0], "__len__") and len(x[0]) == 5:
        x = x[0]
...
def get_gridvals_PICASO_TP():
    ...
    # If set, run explicit case list only (e.g. missing_batchA)
    case_ref = os.environ.get("PICASO_CASE_LIST", "").strip()
    if case_ref:
        cases = _load_cases_from_file(case_ref)  # shape (N,5)
        return (cases,)
...
if __name__ == "__main__":
    ...
    gridutils.make_grid(
        ...
        filename='results/PICASO_climate_updatop_full_exploration_reducedrad_batchB.h5', 
        progress_filename='results/PICASO_climate_updatop_full_exploration_reducedrad_batchB.log'
    ) 
