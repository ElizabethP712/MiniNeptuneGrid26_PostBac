...
start_case.inputs['atmosphere']['exclude_mol'] = {'CH4': 0}
    ...
    if p_surface_bar is not None:
        df_atmo = df_atmo[df_atmo['pressure'] <= p_surface_bar].reset_index(drop=True)
        print(f"Truncated atmosphere to p_surface_bar={p_surface_bar} bar: {len(df_atmo)} layers remaining")
