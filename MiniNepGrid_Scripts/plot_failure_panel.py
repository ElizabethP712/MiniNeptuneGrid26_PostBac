...
def plot_full_pairwise_panel(
    h5_path,
    highlight_inputs_path,
    output_path,
    param_names=None,
    ...
    for i, j in product(range(ndims), repeat=2):
        ...
        plot_slab = slab.astype(float)
        if not show_tp_fail:
            plot_slab[plot_slab == 1] = np.nan
        if not show_incomplete:
            plot_slab[plot_slab == 4] = np.nan

        ax.imshow(plot_slab, origin="lower", aspect="auto", cmap=cmap, norm=norm)
    ...
    if show_tp_fail:
        legend_items.append(Patch(facecolor="#de2d26", edgecolor="none", label="1 TP/PICASO fail"))
    legend_items.append(Patch(facecolor="#fdae6b", edgecolor="none", label="2 Photochem not converged"))
    legend_items.append(Patch(facecolor="#6a51a3", edgecolor="none", label="3 status/error issue"))
    if show_incomplete:
        legend_items.append(Patch(facecolor="#969696", edgecolor="none", label="4 incomplete"))
    if show_highlights:
        legend_items.append(Patch(facecolor="white", edgecolor="black", label="highlighted inputs"))
...
def plot_slice_panel(
    ...
    for ri in range(n_row):
        for ci in range(n_col):
            ax = axs[ri, ci]
            slab = arr[ri, ci].astype(float)  # shape (n_y, n_x)
            if not show_tp_fail:
                slab[slab == 1] = np.nan
            if not show_incomplete:
                slab[slab == 4] = np.nan
            ...
            if show_highlights and highlight_indices is not None:
                mask = (
